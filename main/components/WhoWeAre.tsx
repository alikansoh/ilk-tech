...
export default function WhoWeAre() {
  ...
  const trueImgRef  = useRef<HTMLDivElement>(null);
  const trueTxtRef  = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const scheduleAnimation = () => {
      ...
      (async () => {
        ...
        ctx = gsap.context(() => {
          const shared = (trigger: Element | null) => ({
            duration: 1.0,
            ease: "power2.out",
          ...
          // Arneg section
          gsap.fromTo(imageRef.current,   { x: -40, opacity: 0 }, { x: 0, opacity: 1, ...shared(imageRef.current) });
          gsap.fromTo(textRef.current,    { x:  40, opacity: 0 }, { x: 0, opacity: 1, ...shared(textRef.current) });

          // True Refrigeration section
          gsap.fromTo(trueTxtRef.current, { x: -40, opacity: 0 }, { x: 0, opacity: 1, ...shared(trueTxtRef.current) });
          gsap.fromTo(trueImgRef.current, { x:  40, opacity: 0 }, { x: 0, opacity: 1, ...shared(trueImgRef.current) });
        }, sectionRef);
      })();
  ...
  return (
    ...
    >
      {/* ════════════════════════════════════════════════
          SECTION 1 — ARNEG / WHO WE ARE
      ════════════════════════════════════════════════ */}
      <div className="py-4 sm:py-12 lg:py-10">
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-10 lg:px-14">
        <div className="flex items-center gap-4">
          <div className="h-px flex-1 bg-[#681844]/10" />
          <div className="w-1.5 h-1.5 rotate-45 bg-[#681844]/30" />
          <div className="h-px flex-1 bg-[#681844]/10" />
        </div>
      </div>

      {/* ════════════════════════════════════════════════
          SECTION 2 — TRUE REFRIGERATION
      ════════════════════════════════════════════════ */}
      <div className="py-4 sm:py-12 lg:py-10">
        <div className="max-w-7xl mx-auto px-5 sm:px-10 lg:px-14">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">

            {/* ── TEXT COLUMN (left on desktop — reversed order vs Arneg) ── */}
            <div
              ref={trueTxtRef}
              className="opacity-0 flex flex-col lg:order-1 order-2"
              style={{ willChange: "transform, opacity" }}
            >
              ...
              {/* heading */}
              <h2 className="text-[clamp(2rem,4.2vw,3rem)] font-black leading-[1.1] tracking-[-0.03em] uppercase mb-8">
                <span style={{ color: "#681844" }}>True</span>{" "}
                <span className="text-[#001845]/25">Refrigeration</span>
              </h2>

              {/* rule */}
              <div className="flex items-center gap-4 mb-8">
                <div className="h-px flex-1" style={{ background: "rgba(104,24,68,0.10)" }} />
                <div className="w-1.5 h-1.5 rotate-45" style={{ background: "rgba(104,24,68,0.35)" }} />
              </div>
              ...
              <div className="space-y-5 mb-8">
                <p className="text-[15px] sm:text-[16px] leading-[1.9] text-[#001845]/55 font-light">
                  ...
                  <span className="text-[#001845] font-semibold">Authorised Partner</span>{" "}
                  for{" "}
                  <span className="font-semibold" style={{ color: "#681844" }}>True Refrigeration</span>,
                  one of the world&apos;s leading manufacturers of premium refrigeration equipment.
                </p>
                <p className="text-[15px] sm:text-[16px] leading-[1.9] text-[#001845]/55 font-light">
                  Our partnership allows us to provide customers with industry-leading products
                  renowned for their{" "}
                  <span className="text-[#001845] font-semibold">quality, reliability, energy efficiency,</span>{" "}
                  and{" "}
                  <span className="text-[#001845] font-semibold">innovative design</span>.
                  Whether you&apos;re looking for refrigeration solutions for hospitality, foodservice,
                  retail, or commercial applications, we are committed to helping you find the
                  perfect equipment to meet your needs.
                </p>
                <p className="text-[15px] sm:text-[16px] leading-[1.9] text-[#001845]/55 font-light">
                  Discover the benefits of True Refrigeration and explore our range of products by
                  visiting our dedicated{" "}
                  <Link
                    href="/truerefrigeration"
                    className="font-semibold underline underline-offset-2 transition-colors duration-200"
                    style={{ color: "#681844" }}
                  >
                    True Refrigeration page
                  </Link>{" "}
              ...
              <div className="flex flex-wrap gap-2 mb-10">
                {TRUE_PILLS.map((p) => (
                  <span
                    key={p}
                    className="inline-flex items-center px-3.5 py-2 text-[12px] font-semibold"
                    style={{
                      background: "rgba(104,24,68,0.04)",
                      border: "1px solid rgba(104,24,68,0.12)",
                      color: "rgba(104,24,68,0.70)",
                    }}
                  >
                    {p}
                  </span>
              ...
              <div className="flex flex-wrap gap-3">
                <Link
                  ...
                  style={{
                    border: "1.5px solid #681844",
                    background: "#681844",
                  }}
                  onMouseEnter={(e) => {
                    (e.currentTarget as HTMLAnchorElement).style.background = "white";
                    (e.currentTarget as HTMLAnchorElement).style.color = "#681844";
                  }}
                  onMouseLeave={(e) => {
                    (e.currentTarget as HTMLAnchorElement).style.background = "#681844";
                    (e.currentTarget as HTMLAnchorElement).style.color = "white";
                  }}
                >
                  <span className="absolute inset-0 -translate-x-full group-hover:translate-x-full bg-gradient-to-r from-transparent via-white/10 to-transparent transition-transform duration-700" />
                  <span className="relative flex items-center gap-3">
                    View True Refrigeration
                    <svg
                      className="w-3.5 h-3.5 transition-transform duration-300 group-hover:translate-x-1"
                      fill="none"
                      stroke="currentColor"
                ...
                <a
                  ...
                  className="group inline-flex items-center justify-center gap-2 self-start
                    px-6 py-4 text-[11px] sm:text-[12px] font-black tracking-[0.22em] uppercase
                    transition-all duration-300"
                  style={{
                    border: "1.5px solid rgba(104,24,68,0.25)",
                    color: "#681844",
                    background: "transparent",
                  }}
                  onMouseEnter={(e) => {
                    (e.currentTarget as HTMLAnchorElement).style.borderColor = "#681844";
                    (e.currentTarget as HTMLAnchorElement).style.background = "rgba(104,24,68,0.04)";
                  }}
                  onMouseLeave={(e) => {
                    (e.currentTarget as HTMLAnchorElement).style.borderColor = "rgba(104,24,68,0.25)";
            ...
            <div
              ref={trueImgRef}
              className="relative opacity-0 lg:order-2 order-1"
              style={{ willChange: "transform, opacity" }}
            >
              {/* accent bar — right side to mirror left bar on Arneg section */}
              <div
                className="absolute top-0 -right-3 w-[3px] h-24"
                style={{ background: "#681844" }}
              />

              <div className="relative overflow-hidden aspect-[4/3] shadow-[0_24px_64px_rgba(104,24,68,0.12)]">
                <Image
                  src="/bar.jpeg"
                  alt="True Refrigeration premium equipment"
                  fill
                  quality={85}
                  sizes="(max-width: 1024px) 100vw, 50vw"
                />
                <div
                  ...
                  style={{
                    background: "linear-gradient(to top, rgba(104,24,68,0.55) 0%, transparent 55%)",
                  }}
                />
              </div>
              ...
              <div className="mt-3 flex flex-col gap-2">
                ...
                <div
                  ...
                  style={{
                    background: "rgba(255,255,255,0.97)",
                    border: "1px solid rgba(104,24,68,0.10)",
                    backdropFilter: "blur(14px)",
                    WebkitBackdropFilter: "blur(14px)",
                    boxShadow: "0 8px 32px rgba(104,24,68,0.08)",
                  }}
                >
                  <div
                  ...
                  <div>
                    <p className="text-[#001845] font-bold text-sm leading-tight">
                      Authorised{" "}
                      <span style={{ color: "#681844" }}>True Refrigeration</span>{" "}
                      Partner
                    </p>
                    <p className="text-[#001845]/45 text-xs mt-0.5 font-medium">Authorised Distribution Partner</p>
                  </div>
                ...
                {/* feature tags */}
                <div className="flex gap-2">
                  {(["Energy Efficient", "Industry Leading"] as const).map((label) => (
                    <div
                      key={label}
                      className="flex flex-1 items-center gap-3 px-4 py-3"
                      style={{ background: "#681844" }}
                    >
                      <span
                        className="w-[3px] h-8 flex-shrink-0"
                        style={{ background: "rgba(255,255,255,0.35)" }}
                      />
                      <div>
                        <p className="text-white font-black text-sm leading-none">{label}</p>
                        <p
